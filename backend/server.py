...
def build_evidence_ledger(session: dict, graph: dict, posts: list, stock_data: list, report: dict) -> list:
    """Create a transparent list of sources and how each influenced the report."""
    ledger = []
    if session.get("intel_brief"):
        ledger.append({
            "source": "Live intelligence brief",
            "type": "web_intelligence",
            "freshness": session.get("live_fetched_at") or session.get("updated_at"),
            "used_for": ["context", "stakeholders", "knowledge graph"],
            "confidence_impact": "+20%",
            "notes": "Current web/news context was used to ground the seed world.",
        })
    ...
    if entities or relationships:
        ledger.append({
            ...
            "freshness": session.get("updated_at"),
            "used_for": ["persona generation", "report framing"],
            "confidence_impact": "+10%",
            "notes": f"{len(entities)} entities and {len(relationships)} relationships shaped the simulation world.",
        })

    simulated_posts = [post for post in posts if not post.get("is_real")]
    if simulated_posts:
        sentiment = aggregate_sentiment([post.get("content", "") for post in simulated_posts])
        ledger.append({
            "source": "Simulated agent posts",
            "type": "simulation_trace",
            "freshness": max((post.get("created_at", "") for post in simulated_posts), default=None),
            "used_for": ["opinion landscape", "factions", "turning points"],
            "confidence_impact": f"+{round(min(0.20, len(posts) / 500) * 100)}%",
            "notes": f"{len(simulated_posts)} simulated posts; mean valence {sentiment['mean_valence']:+.2f}.",
        })

    real_seed_posts = [post for post in posts if post.get("is_real")]
    real_sentiment = session.get("social_seed_sentiment")
    if real_seed_posts or real_sentiment:
        ledger.append({
            "source": "Real social seed",
            "type": "social_grounding",
            "freshness": session.get("updated_at"),
            "used_for": ["calibration", "real-vs-simulated drift"],
            "confidence_impact": "+10%" if real_sentiment else "+5%",
            "notes": f"{len(real_seed_posts)} real seed posts; sources: {', '.join(session.get('social_seed_sources', []) or ['unknown'])}.",
        })

    for item in stock_data or []:
        ledger.append({
            "source": f"{item.get('name', item.get('ticker', 'Market data'))} ({item.get('ticker', 'N/A')})",
            "type": "market_data",
            "freshness": item.get("fetched_at"),
            "used_for": ["financial grounding", "technical signals"],
            "confidence_impact": "+15%",
            "notes": f"Last close {item.get('last_close')} {item.get('currency', '')}; trend {item.get('trend', 'unknown')}.",
        })

    quality = report.get("prediction_quality", {})
    if quality.get("simulation_reliability", {}).get("fallback_posts"):
        fallback_posts = quality["simulation_reliability"]["fallback_posts"]
        ledger.append({
            "source": "Deterministic fallback generation",
            "type": "reliability_control",
            "freshness": session.get("updated_at"),
            "used_for": ["simulation continuity"],
            "confidence_impact": f"-{round(min(0.20, quality['simulation_reliability'].get('fallback_rate', 0) * 0.5) * 100)}%",
            "notes": f"{fallback_posts} posts were generated by fallback logic after malformed LLM output.",
        })

    return ledger


def build_ensemble_forecast(posts: list, report: dict, runs: int = 25) -> dict:
    """Bootstrap the simulated trace into an ensemble-style consensus and stability score."""
    simulated_posts = [post for post in posts if not post.get("is_real") and post.get("content")]
    if not simulated_posts:
        return {
            "runs": 0,
            "method": "Bootstrap resampling of simulation posts",
            "dominant_outcome": "insufficient_data",
            "stability_score": 0,
            "consensus": {"support": 0, "opposition": 0, "undecided": 0},
            "variance": {"support": 0, "opposition": 0, "undecided": 0},
            "interpretation": "Not enough simulated posts to build an ensemble forecast.",
        }

    rng = random.Random(20260426)
    sample_size = max(5, min(len(simulated_posts), round(len(simulated_posts) * 0.7)))
    outcomes = []
    for run_idx in range(runs):
        sample = [rng.choice(simulated_posts) for _ in range(sample_size)]
        sentiment = aggregate_sentiment([post.get("content", "") for post in sample])
        support = sentiment["positive"]
        opposition = sentiment["negative"]
        undecided = sentiment["neutral"]
        dominant = max(
            [("support", support), ("opposition", opposition), ("undecided", undecided)],
    ...
    def mean(field):
        return round(sum(item[field] for item in outcomes) / len(outcomes))

    def variance(field):
        avg = sum(item[field] for item in outcomes) / len(outcomes)
        return round(sum((item[field] - avg) ** 2 for item in outcomes) / len(outcomes), 1)

    consensus = {
        "support": mean("support"),
        "opposition": mean("opposition"),
        "undecided": mean("undecided"),
    }
    dominant_outcome = max(consensus, key=consensus.get)
    dominant_share = sum(1 for item in outcomes if item["dominant"] == dominant_outcome) / len(outcomes)
    dispersion = (variance("support") + variance("opposition") + variance("undecided")) / 3
    stability_score = round(max(0, min(100, dominant_share * 100 - min(35, dispersion / 4))))

    return {
        "runs": runs,
        "method": "Bootstrap resampling of simulated posts",
        "dominant_outcome": dominant_outcome,
        "stability_score": stability_score,
        "consensus": consensus,
        "variance": {
            "support": variance("support"),
            "opposition": variance("opposition"),
            "undecided": variance("undecided"),
        },
        "sample_size": sample_size,
        "interpretation": (
            "High agreement across bootstrap runs." if stability_score >= 75 else
            "Moderate agreement; monitor evidence shifts." if stability_score >= 50 else
            "Low stability; treat the outcome as exploratory."
        ),
        "runs_preview": outcomes[:8],
        "base_prediction": report.get("prediction", {}).get("outcome"),
    }
...
async def fetch_nitter_rss(topic: str, limit: int = 15) -> list:
    ...
    try:
        ...
        for nitter_url in nitter_urls:
            try:
                req = urllib.request.Request(nitter_url, headers={"User-Agent": "Predicta/1.0"})
...
async def download_report_pdf(session_id: str):
    ...
    pdf.cell(0, 15, "Predicta Prediction Report", ln=True, align="C")
    pdf.ln(5)
    ...
    pdf.cell(0, 5, "Generated by Predicta - Evidence-Aware Prediction Engine", ln=True, align="C")
    if stock_data:
    ...
    filename = f"predicta_report_{session_id[:8]}_{datetime.now().strftime('%Y%m%d')}.pdf"
...
Stay completely in character. Be opinionated. 2-4 sentences per reply. Do not reveal you are an AI."""
        ...
        system_prompt = f"""You are the Predicta ReportAgent — an expert analyst who completed a multi-agent social simulation.
Prediction Question: {query}
